...
/**
 * Holds information of the node package
 *
 * @class Package
 */
 ...
 */
function Package(opts, collection, packageDirectory) {
...
Package.prototype = {
    ...
    addDependencies: function () {
        for (var name in this.dependencies) {
            this.collection.add(name, path.join(this.packageDirectory, name));
        }
    },
    ...
    getFiles: function (force) {
        ...
        main.forEach(function (pattern) {
            if (pattern[0] === '/') {
                throw new Error('absolute path in package main is not supported');
            }
