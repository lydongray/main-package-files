...
describe('main-package-files', function () {
    function expect(filenames) {
        ...
        function run(path, options, done) {
            ...
            if (!options.paths.packageJson) {
                options.paths.packageJson = __dirname + path;
            }

            if (!options.paths.package) {
                options.paths.package = __dirname + '/package.json';
            }

            if (!options.paths.packageDirectory) {
                options.paths.packageDirectory = 'test/fixtures';
            }

            if (typeof filter === 'string' || Array.isArray(filter) || filter instanceof RegExp) {
                delete options.filter;
                srcFiles = mainPackageFiles(filter, options, options.callback || undefined);
            } else {
                srcFiles = mainPackageFiles(options, options.callback || undefined);
            }
    ...
    it('should select the expected files', function (done) {
        expect([
            '/fixtures/nested/nest/release/nested.js',
        ...
        ]).fromConfig('/_package.json').when(done);
    });

    it('should select the expected files with dependency override', function (done) {
        expect([
            '/fixtures/simple2/simple2.js',
            '/fixtures/simple/simple.js',
            '/fixtures/nested/nest/release/nested.js'
        ]).fromConfig('/_package2.json', {
            overrides: {
                'nested/nest': {
                    dependencies: {
                        "simple": "*"
    ...
    it('should select only files that pass a given filter regular expression', function (done) {
        expect([
            ...
            '/fixtures/deepPaths/lib/deeppaths.js',
            '/fixtures/decoy/decoy.js'
        ]).fromConfig('/_package.json', { filter: /\.js$/i }).when(done);
    });
    ...
    it('should select only files that pass a given glob filter', function (done) {
        expect([
            ...
            '/fixtures/deepPaths/lib/deeppaths.js',
            '/fixtures/decoy/decoy.js'
        ]).fromConfig('/_package.json', { filter: '**/*.js' }).when(done);
    });
    ...
    it('should select the expected files with relative path', function (done) {
        expect([
            ...
            paths: {
                packageJson: './test/_package.json',
    ...
    it('should ignore packages without any json files', function (done) {
        expect([
            '/fixtures/simple/simple.js'
        ]).fromConfig('/_nojson_package.json').when(done);
    });
    ...
    it('should select files via default option', function (done) {
        expect([
            ...
            '/fixtures/simple/simple.js'
        ]).fromConfig('/_nojson_package.json', { main: './**/*.js' }).when(done);
    });
    ...
    it('should recurse through dependencies pulling in their dependencies', function (done) {
        ...
        ]).fromConfig('/_recursive_package.json').when(done);
    });
    ...
    it('should not get hungup on cyclic dependencies', function (done) {
        ...
        ]).fromConfig('/_cyclic_package.json').when(done);
    });
    ...
    it('should get devDependencies', function (done) {
        ...
        ]).fromConfig('/_includedev_package.json', { includeDev: true }).when(done);
    });

    it('should get only devDependencies', function (done) {
        expect([
            '/fixtures/includeDev/includeDev.js'
        ]).fromConfig('/_includedev_package.json', { includeDev: 'exclusive' }).when(done);
    });

    it('should get devDependencies and ignore missing dependencies', function (done) {
        expect([
            '/fixtures/includeDev/includeDev.js'
        ]).fromConfig('/_includedev_devdepsonly_package.json', { includeDev: true }).when(done);
    });

    it('should get main file', function (done) {
        expect([
            'main.js'
        ]).fromConfig('/_includeSelf_package.json', { includeSelf: true }).when(done);
    });

    it('should not load any deeper dependencies', function (done) {
        expect([
            '/fixtures/recursive/recursive.js'
        ]).fromConfig('/_dependencies_package.json').when(done);
    });
    ...
    it('should load other dependencies than defined', function (done) {
        expect([
            '/fixtures/decoy/decoy.js',
            '/fixtures/recursive/recursive.js'
        ]).fromConfig('/_other_dependencies_package.json').when(done);
    });
    ...
    it('should select prod.js on prod environment', function (done) {
        ...
        ]).fromConfig('/_env_based_package.json').when(done);
    });
    ...
    it('should throw an exception if package.json does not exists', function () {
        var when = expect([]).fromConfig('/_unknown.json').when;

        when.should.throw();
    });

    it('should pass an error to callback if package.json does not exists', function () {
    ...
    it('should select the expected files with comments in the package.json', function (done) {
        expect([
            '/fixtures/overwritten/another.js'
        ]).fromConfig('/_package_with_comments.json').when(done);
    });
    ...
    it('should select all files if no group options pass to function', function (done) {
        expect([
            ...
            '/fixtures/deepPaths/lib/deeppaths.js',
            '/fixtures/decoy/decoy.js'
        ]).fromConfig('/_package_with_group.json').when(done);
    });

    it('should select the expected files from group property in package.json', function (done) {
        expect([
            '/fixtures/simple/simple.js',
            '/fixtures/multi/multi.js',
            '/fixtures/multi/multi.css'
        ]).fromConfig('/_package_with_group.json', { group: 'group1' }).when(done);
    });

    it('should select the expected files from group property in package.json defined by array of groups', function (done) {
        expect([
            '/fixtures/simple/simple.js',
            '/fixtures/multi/multi.js',
            '/fixtures/multi/multi.css',
            '/fixtures/overwritten/overwritten.js'
        ]).fromConfig('/_package_with_group.json', { group: ['group1', 'group2'] }).when(done);
    });

    it('should select all files except those listed in the group property in package.json', function (done) {
        expect([
            '/fixtures/overwritten/overwritten.js',
            '/fixtures/deepPaths/lib/deeppaths.js',
            '/fixtures/decoy/decoy.js'
    ...
    it('should throw an exception if group name does not exist', function () {
        var when = expect([]).fromConfig('/_package_with_group.json', { group: 'nonExistingGroup' }).when;

        when.should.throw('group "nonExistingGroup" does not exists in package.json');
    });

    it('should ignore nonexisting packages in the group', function (done) {
        expect([
            '/fixtures/simple/simple.js'
        ]).fromConfig('/_package_with_group.json', { group: 'containDepsError' }).when(done);
    });

    it('should not giving error wrong error message when a / path was passed', function (done) {
        var when = expect([
        ]).fromConfig('/_package_with_wrong_main_path.json', { checkExistence: true }).when;

        when.should.throw('absolute path in package main is not supported');
        done();
    });
});
